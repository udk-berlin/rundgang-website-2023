    ...
    <SavedProjectsContainer>
      ...
      <SavedProjectsSVGContainer numberOfSavedProjects={numberOfSavedProjects}>
        <SavedProjectsLink setIsLinkClicked={setIsLinkClicked}>
          ...
          >
            <SVG
              src={`/assets/svg/layout/shop_state_${numberOfSavedProjectsForPath}${
                isActive && numberOfSavedProjectsForPath > 0 ? "_active" : ""
              }.svg`}
            />
          </div>
        </SavedProjectsLink>
...
const SavedProjectsSVGContainer = styled.div`
  margin-top: calc(
    0.8 * var(--layout-header-bar-container-height) *
      ${({ numberOfSavedProjects }) =>
        numberOfSavedProjects in numberOfSavedProjectsToMarginTopMapper
          ? numberOfSavedProjectsToMarginTopMapper[numberOfSavedProjects]
          : numberOfSavedProjectsToMarginTopMapper[3]} * -1
  );
`;

const SVG = styled(ReactSVG)`
  width: calc(0.7 * var(--layout-header-bar-container-height));
  height: calc(0.7 * var(--layout-header-bar-container-height));
  cursor: pointer;
