...
export default function TimelineHours() {
  ...
  return (
    <HoursContainer>
      <HoursInnerContainer>
        {useHalfDayHours ? <HalfDayHours /> : <AllHours />}
      </HoursInnerContainer>
    </HoursContainer>
  );
}
    ...
    <>
      {range(0, NUMBER_OF_HOURS).map((hourIndex) => {
        ...
        if (hourIndex > 1 && hourIndex < NUMBER_OF_HOURS - 1) {
          let hour =
            (hourIndex + FIRST_DAY_START_HOUR) %
            (language.locale === "en" ? HOURS_PER_HALF_DAY : HOURS_PER_DAY);
          if (
            hour === 0 &&
            language.locale === "en" &&
            (hourIndex + FIRST_DAY_START_HOUR) % HOURS_PER_DAY !== 0
          ) {
          ...
          content = (
            <>
              ...
              <Hour>
                {hour}
                {language.locale === "en"
                  ? (hourIndex + FIRST_DAY_START_HOUR) % HOURS_PER_DAY <
                    HOURS_PER_HALF_DAY
                    ? "am"
                    : "pm"
                  : ":00"}
        ...
        return <HourContainer>{content}</HourContainer>;
      })}
    </>
  );
}
  ...
  return (
    <>
      {range(0, NUMBER_OF_HALF_DAYS).map((halfDay) => {
        let content;
        let hour =
          (halfDay * HOURS_PER_HALF_DAY + FIRST_DAY_START_HOUR + 2) %
          HOURS_PER_DAY;
        ...
        content = (
          <>
            ...
            <Hour>
              {hour}
              {language.locale === "en"
                ? hour % HOURS_PER_DAY < HOURS_PER_HALF_DAY
                  ? " am"
                  : " pm"
                : ":00"}
            </Hour>
...
const HourLine = styled.div`
  ...
  margin-bottom: calc(
    calc(
        ${({ theme }) => theme.hours.height} - 2 *
          ${({ theme }) => theme.borderWidth}
      ) / 2
  );
  border-bottom: ${({ theme, transparent }) =>
    transparent ? 0 : `${theme.borderWidth} solid ${theme.colors.pink}`};
`;
